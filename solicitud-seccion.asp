...
if EstaHabilitadaNW (473)="S" then 
	if GetPermisoNW(473) ="N" then
		...
		%><script>window.top.location.href ='MensajesBloqueos.asp';</script><%
	else
		if TomadeRamoED(session("Codcarr"),session("Codcli"),session("ano_ed"),session("periodo_ed"))=0 then
			...
			%><script>window.top.location.href ='MensajeBloqueo.asp';</script><%
		end if 
...
Audita 473,"Ingresa a solicitud especial de Asignaturas"
  ...
  StrSql = "SELECT dbo.Fn_ValorParame('ObligaRamosFueraMalla') as ObligaRamosFueraMalla  WHERE dbo.Fn_ValorParame('ObligaRamosFueraMalla') ='S' "' from mt_parame"
'  StrSql = StrSql & " where ObligaRamosFueraMalla='S'"
  ...
  'StrSql = StrSql & " AND codramo IN ( SELECT codramo FROM ra_nota WHERE Codcli = '" & Codcli & "' " 
...
{ var j, k;
   ...
   if ((Obj.checked) && (Obj.value == "false") )
   {
      ...
      parent.Horario.location = "GrabaSol.asp?A=D&R=" + Ramo + "&S=" + Seccion + "&C=" + Curso + "&CodCarr=" + CodCarr
      <% else %>
   ...
   if ((Obj.checked) && (Obj.value != "false") )
     {
      <% if SoloPrueba = "N" then %>
       parent.Horario.location = "GrabaSol.asp?A=I&R=" + Ramo + "&S=" + Seccion + "&T=" + Obj.form.elements["T" + Curso].value + "&C=" + Curso + "&CodCarr=" + CodCarr
      <% else %>
...
{
    window.location = "solicitud-seccion.asp?P=<%=var%>&Niv=" + nivel;
}
function RefrescarJornada(Jornada)
{
    window.location = "solicitud-seccion.asp?P=<%=var%>&Niv=" + nivel + "&J=" + Jornada;
}
      ...
      <tr>
         <td height="30" align="left" valign="top"><% CargarTop1()%><% SubMenu()%>
		    <table width="750" border="0" cellspacing="15" cellpadding="0" height="43">
                ...
                <tr>
                  <%if var="S" then %>
                  <td width="336" valign="bottom" height="34" class="tex-normales"> <font id="lblAsigJornada">Asignaturas Jornada</font>
                    <select name="SelJornada" onclick="javascript:RefrescarJornada(this.value);" onChange="javascript:Actualizar();">
                       <% StrJor = "Select jornada,descripcion from ra_jornada"
					      if BCL_ADO(StrJor, jor) then 
						  While Not jor.Eof %>
						  <option value=<%=jor("jornada")%> selected><%=jor("descripcion")%></option>						  
						 <% jor.MoveNext 
						  wend
						  end if %>
                  ...
                  <td width="336" valign="bottom" height="34" class="tex-normales"><%end if%>
                      <font id="lblNivel">Seleccione Nivel</font>
        <%for i = ValNulo((Min), num_) to ValNulo((Max), num_) %>
                  ...
                  <td width="248" height="34"><%if var="S" then %>
                    ...
                    <% else %>
                    <font id="lblAsigJornada" style="display:none"></font>
...
'response.Write("OTROSRAMOS '" + Codcli + "' ,  '"&  NivelChosen & "'" )
'response.End()
...
%>
              <% ' Recorremos las secciones 
				 ...
				 'response.Write("Ramo" & ArregloOtrosRamos(0, posOtros))
                 'Response.End 
                   ...
                   case "OBLIGATORIO"
                     if EsPlanificado(ArregloOtrosRamos(0, posOtros), CodSede, Ano, Periodo,codcarr) then
                       ...
                       if Trim(Equivalente) <> "" then
						  ...
						  'Mejora 2.0,  SEGUIR !!
						  StrSeccion = StrSeccion & " Where e.CodRamo in (Select a.ramoequiv from ra_equiv a with (nolock), mt_carrer b with (nolock) Where a.codcarr = b.codcarr And b.Sede = '" & CodSede & "' And a.CodRamo = '" & ArregloOtrosRamos(0, posOtros) & "' and a.codcarr = '" & trim(CodCarr) & "')" 					    
...
%>
			  ...
			  <tr valign="top"> 
			    <td><table width="750" cellspacing="1" cellpadding="0" height="19" border="0" bordercolor="#FFFFFF">
                  <tr>
                    <td height="18" colspan="7" background="Imagenes/fdo-cabecera-cel2.jpg"><div align="left" class="text-cabecera-celda"><font c ><b><font size="2" name="lblEncabezadoRamo">Asignatura</font></b></font><font c ><b><font size="2">:
                  ...
                  <tr class="text-cabecera-celda" background="imagenes/fdo-cabecera-cel22.jpg" >
                    ...
                    <td width="10%" height="30" background="imagenes/fdo-cabecera-cel22.jpg"><div align="center"><font face="Verdana, Arial, Helvetica, sans-serif" color="#FFFFFF"><b><font size="1" face="Geneva, Arial, Helvetica, san-serif" name="lblColumnaProfesor">Profesor</font></b></font></div></td>
                    ...
                    <td width="30%" height="30" background="imagenes/fdo-cabecera-cel22.jpg"><div align="center"><font face="Verdana, Arial, Helvetica, sans-serif" color="#FFFFFF"><b><font size="1" face="Geneva, Arial, Helvetica, san-serif" name="lblColumnaRamo">Ramo</font></b></font></div></td>
...
<%ObjetosLocalizacion("solicitud-seccion.asp")%>
</html>
