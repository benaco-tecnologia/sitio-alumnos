...
Audita 488,"Ingresa a Descarga de apuntes" 
                      ...
                      <table width='100%' border='1' cellpadding='3' cellspacing='0' class='smalltext'>
                        <tr>
						  ...
						  if DOCTOSENACPA="SI" then
								  strD0="select DISTINCT CodProf from ca_doctos where codramo='" & Rs2("CODRAMO") & "' and AnoPublicacion= '"&trim(ano)&"' And PeriodoPublicacion='1' And Activo='1' and codcarr in (select codcarr COLLATE Modern_Spanish_CI_AS from mt_Carrer where sede='"& session("codsede") &"') AND COALESCE(EVALUADO,'NO')<>'SI'"
                      ...
                      <table width='100%' border='1' cellpadding='3' cellspacing='0' class='smalltext'>
                        <tr>
                          <td bgcolor='#FFFFCC'><span class="normalname"><a style='text-align:left; cursor:hand; cursor:pointer; text-decoration:none;' onClick="expandit('<%=Rs2("CODRAMO")&"_" & Rs2("codsecc")& "_" & Rs2("PERIODO")& "_" &Rs2("ANO")%>');"><%
						 	...
						 	if DOCTOSENACPA="SI" then
								  strD0="select DISTINCT CodProf from ca_doctos where codramo='" & Rs2("CODRAMO") & "' and AnoPublicacion= '"&trim(ano)&"' And PeriodoPublicacion='2' And Activo='1' and codcarr in (select codcarr COLLATE Modern_Spanish_CI_AS from mt_Carrer where sede='"& session("codsede") &"') AND COALESCE(EVALUADO,'NO')<>'SI'"
...
if DOCTOSENACPA="SI" then 
	...
	strD = strD & "EVALUADO,FECHA_INI,FECHA_FIN,ACEPTA_MAT_FUERA_PLAZO from ca_doctos where codramo='" & Rs2("CODRAMO") & "' and CodProf='" & Rs6("CodProf") & "' and AnoPublicacion='"&trim(ano)&"' "
	strD=strD & "And PeriodoPublicacion='2' And Activo='1' and codcarr in (select codcarr COLLATE Modern_Spanish_CI_AS from mt_Carrer where sede='"& session("codsede") &"') AND COALESCE(EVALUADO,'NO')<>'SI' "
	if MaterialSeccion="SI" then
		strD=strD & "and codsecc='"& Rs2("codsecc") &"'"
	end if
