...
Case "GET_TTL"
	...
	Select Case VFMT
		...
		Case "ENCP"
			response.binarywrite goPageGenerator.RenderTotallerETF(gvGroupPath, 0, 0, gvMaxNode, 8209)
...
Case "RFSH"
	...
	Select Case VFMT
	Case "ENCP"
	' Active X Viewer will make get page command after receiving 0.  Java Viewer must have page sent back.
		if VIEWER = "JAVA" then
			response.binarywrite goPage.Renderepf(8209)
		else
...
Case "SRCH"
	...
	Case "HTML_FRAME"
		...
		If(CBool(goPageGenerator.SearchForText(TEXT, 0, tempNumber))) then
...
SUB CheckForError()
	If Err.Number <> 0 Then
		if VFMT = "ENCP" then
			Response.ContentType = EMFMIMETYPE
			session("oEMF").SendErrorMsg 1, "CPEAUT Error Occured on Server. " & Err.Number  & " : " & Err.Description
		else
			Response.Write "CPEAUT Error Occured on Server. Error Number: " & Err.Number & " Error Description: " & Err.Description
		end if
		Response.end
	End if
END SUB
