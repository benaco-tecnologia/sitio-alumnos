...
<script languaje="javascript">
		...
		var DebesNoInscritos = false;
		  ...
		  for (i = 0; i <= <%=Ubound(RamosSelec, 1)%>; i++)
		  {
		   ...
		   {window.location="asignatura-seccion-especial-nivel.asp?Ramos=<%=Ramos%>";}
		}
--></script>
		  ...
		  <tr>
			...
			<% CargarTop1()%><% SubMenu()%>
			 <table width="762" border="0" cellspacing="15" cellpadding="0">
				  <tr> 
					<td width="436"><img src="Imagenes/titulos/T-inscrip_especial_asig.jpg" width="415" height="38"></td>
					<td width="281">&nbsp;</td>
				  </tr>
				  <%
			  ...
			  if CantidadRamos > 1 then
					...
					%>
						  <tr> 
								    ...
								    <% Response.Write("<font size='1' face='Arial, Helvetica, sans-serif'>")
												 for i = 0 to ubound(RamosSelec, 1) - 1
													Response.Write("<a href='asignatura-seccion-especial-nivel.asp?P=N&Niv=" & i & "&CodCarr2=" & CodCarr2 & "'>" & RamosSelec(i) & "</a>&nbsp&nbsp ")
												 next
												 Response.Write("</font>")
										  %>									</td>
						...
						<%
						 ...
						 %>
							...
							<td> 
							  <div align="right">
							  <a href="inscrip-asigna.asp" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image1','','Imagenes/botones/continuar-on.gif',1)"><img src="Imagenes/botones/continuar-of.gif" width="178" height="45" name="Image1" border="0"></a></div>							</td>
							</tr>
					 <%  
						   end if
			else
				%>
						<td height="21"> 
						  <p><font size="2" face="Verdana, Arial, Helvetica, sans-serif"><b><font size="1"><!--a href="javascript:;" onMouseOver="MM_openBrWindow('tex-seccionhorario.htm','seccionhorario','width=500,height=500')"><img src="imag/botones/instrucciones.gif" width="122" height="23" border="0"></a--></font></b></font></p>						</td>
						<td> 
						  <div align="right"><a href="inscrip-asigna.asp" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image1','','Imagenes/botones/continuar-on.gif',1)"><img src="Imagenes/botones/continuar-of.gif" width="178" height="45" name="Image1" border="0"></a>						  </div>						</td>
					</tr>
				  <%          
			end if  
			%>
						...
						<td width="72" height="16" > <div align="center">
							<p><font face="Verdana" size="1"> 
							  ...
							  <% if EstaPreInscrito(Codcli, RamosSelec(i), ArrayRamos(0, posRamos), ArrayRamos(1, posRamos), Ano, Periodo, TipoCurso) then %>
									...
									<input type="radio" name="<%=RamosSelec(iP)%>" value="<%=ArrayRamos(0, posRamos)%>" OnFocus="javascript:marca(this, '<%=trim(ArrayRamos(0, posRamos))%>')" OnClick="javascript:GrabaSel(this,'<%=RamosSelec(i)%>', '<%=ArrayRamos(0, posRamos)%>', '<%=ArrayRamos(1, posRamos)%>',<%=i%>,'<%=mid(TextoTipoCurso ,1,1)%>','<%=ArrayRamos(9, posRamos)%>')" CHECKED >
							  <% else %>
									<input type="radio" name="<%=RamosSelec(iP)%>" value="<%=ArrayRamos(0, posRamos)%>" OnFocus="javascript:marca(this, '<%=trim(ArrayRamos(0, posRamos))%>')" OnClick="javascript:GrabaSel(this, '<%=RamosSelec(i)%>', '<%=ArrayRamos(0, posRamos)%>', '<%=ArrayRamos(1, posRamos)%>',<%=i%>,'<%=mid(TextoTipoCurso ,1,1)%>','<%=ArrayRamos(9, posRamos)%>')"  >
							  
							  <% end if %>
							  </font> </p>
						</div></td>
						<td width="101" ><center> <p><font face="Verdana" size="1"><%=TextoTipoCurso%></font><font face="Verdana" size="1"></font></p></center></td>
						<td width="57" valign="top"> <p><font face="Verdana" size="1"></font><font face="Verdana" size="1"></font></p></td>
					  </tr>
					  <%		posRamos = posRamos + 1
